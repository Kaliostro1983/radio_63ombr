...
def _run_lightweight_migrations(conn: sqlite3.Connection) -> None:
    ...
    for _i, _reason in enumerate(
        ["фпв", "підрив на міні", "стрілецький бій", "скид", "аварія",
         "самоушкодження", "невідомо", "осколкове поранення", "хвороба", "отруєння"]
    ):
        safe_execute(
            conn,
    ...
    # Ознака «пеленг отримано імпортом з файлу» (CSV), а не введено вручну.
    # Саме на ТОЧЦІ, а не на батчі: імпорт дописує точки в наявний батч
    # (event_dt+network_id), тож один пеленг може мати і ручні, і імпортовані.
    _ensure_column(conn, "peleng_points", "is_imported",
