...
from app.core.access import VALID_ROLES, is_admin
...
def _is_local(request: Request) -> bool:
    """Break-glass: керування завжди доступне з 127.0.0.1 (SSH-тунель)."""
    return (request.client.host if request.client else "") in _LOCAL_HOSTS


def _require_admin_api(request: Request):
    """Гейт для JSON-ендпоінтів: адмін-пристрій або localhost."""
    if not (is_admin(request) or _is_local(request)):
        raise HTTPException(status_code=403, detail="Лише для адміністратора")


def _page_guard(request: Request):
    """Гейт для HTML-сторінок: адмін-пристрій або localhost, інакше 403."""
    if is_admin(request) or _is_local(request):
        return None
    return HTMLResponse(
        "403 — доступ лише з пристрою з роллю admin (або з 127.0.0.1).",
        status_code=403,
    )
