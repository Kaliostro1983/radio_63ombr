...
_EXPORT_SIDC = "10016600006099000000"
# Verbatim AlpineQuest generator note (kept exactly as in source files).
_EXPORT_GEN_DATA = (
    '                <Data name="Generated by AlpineQuest©&lt;br /&gt;&lt;a href=&quot;https">\n'
    '                    <value>//www.alpinequest.net" alt="https://www.alpinequest.net"&gt;'
    'https://www.alpinequest.net&lt;/a&gt;</value>\n'
    '                </Data>\n'
)


def _export_icon_bytes() -> bytes:
    """Marker icon copied into the KMZ once per point (the exact icon the source
    palettes use). Without it the target program shows a '?' placeholder."""
    try:
...
def _build_palette_kml(points):
    """Build doc.kml in the AlpineQuest palette format and return
    (kml_text, [png_href, ...]) — ONE per point. Each point gets its own
    Style id, Placemark id and files/<uuid>.png reference (the target program
    needs per-point styles + icons, not a shared one). Inline IconStyle colour
    tints the icon; <ExtendedData>/sidc identifies the symbol.
    """
    esc = html.escape
    styles = []
    placemarks = []
    ...
    for p in points:
        uid = str(uuid.uuid4())
        href = f"files/{uid}.png"
        png_hrefs.append(href)
        code = esc(str(p["code"] or ""))
        kcol = _hex_to_kml_color(p["color"])
        lat = p["lat"]; lon = p["lon"]
        styles.append(
            f'        <Style id="{uid}-style">\n'
            '            <IconStyle>\n'
            '                <scale>1.3</scale>\n'
            '                <heading>0.0</heading>\n'
            '                <Icon>\n'
        ...
        placemarks.append(
            ...
            '            <description>Точка</description>\n'
            f'            <styleUrl>#{uid}-style</styleUrl>\n'
            '            <Style>\n'
            '                <IconStyle>\n'
            f'                    <color>{kcol}</color>\n'
            '                    <scale>0.0</scale>\n'
...
def api_palette_export(palette_id: int):
    ...
    def _kmz_bytes(points) -> bytes:
        kml, png_hrefs = _build_palette_kml(points)
        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as z:
            z.writestr("doc.kml", kml.encode("utf-8"))
            if icon_bytes:
                # One icon copy per point (files/<uuid>.png), as in source files.
                for href in png_hrefs:
                    z.writestr(href, icon_bytes)
        return buf.getvalue()
